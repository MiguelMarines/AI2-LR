...
def scaling_function(x_features):
	...
	for i in range(1, len(x_features)):
		...
		# Minimum - Maximum Normalization = (X - Xmin) / (Xmax - Xmin)
		for j in range(len(x_features[i])):
...
x_features = np.loadtxt('/Users/miguelangelmarinesolvera/Desktop/AI2-LR/nuevo_leon.data', dtype = int, delimiter = ',')
...
y_results = np.loadtxt("/Users/miguelangelmarinesolvera/Desktop/AI2-LR/nuevo_leon.data", usecols = 3, dtype = int, delimiter = ",")
# Print y (target).
#print(y_results)
...
# Scale the data set with minimum - maximum norrmalization.
